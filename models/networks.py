...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    # elif netF == 'mlp_sample_1024':
    #     net = PatchSampleF_1024(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
...
def define_H(input_nc, output_nc, ngf, netH, norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, no_antialias_up=False, gpu_ids=[], opt=None):

    norm_layer = get_norm_layer(norm_type=norm)

    if netH == 'H1':
        net = pdfam_gau_module1()
    elif netH =='H2':
        net = pdfam_gau_module2()
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % netH)
    return init_net(net, init_type, init_gain, gpu_ids, initialize_weights=('stylegan2' not in netH))
...
#             if num_patches > 0:
#                 if patch_ids is not None:
#                     # 1. 根据 feak_B获取到对应A的patch_id
#                     patch_id = patch_ids[feat_id]
#                     # 2. 得到patch_id原来的数据范围，以便接下来不取到重复的数
#                     total_range = set([x for x in range(0,feat_reshape.shape[1])])
#                     patch_id_tmp=set(patch_id.tolist())
#                     ramain_number = list(total_range-patch_id_tmp)
#                     # 3.根据得到的不重复的剩余的数，获取新的id
#                     new_supplementary_id = torch.randperm(len(ramain_number), device=feats[0].device)
#                     # new_supplementary_id = new_supplementary_id[:3*256]#
#                     new_supplementary_id = new_supplementary_id[:3*num_patches]#
#                     # 4. 使用fake_b传过来的ID和新的id选取patch
#                     x_sample = feat_reshape[:, patch_id, :].flatten(0, 1)  # reshape(-1, x.shape[1])
#                     new_sample = feat_reshape[:, new_supplementary_id, :].flatten(0, 1)
#                     # 5. 比较相似性,给x_sample中的每个行向量选3个跟他最相似的
#                     x_sample_normalized = F.normalize(x_sample, p=2, dim=1)
#                     x_new_sample_normalized = F.normalize(new_sample, p=2, dim=1)
#                     similarity = torch.mm(x_sample_normalized, torch.t(x_new_sample_normalized))
#                     values_per_row = []
#                     # 这个位置就对应着x_new_sample_normalized转置的位置,也就是new_sample行的位置
#                     indices_per_row = []
#                     for row in similarity:
#                         values, indices = torch.topk(row, 3)
#                         values_per_row.append(values)
#                         indices_per_row.append(indices)
#                     # 6. 在new_sample中取出x_sample每行对应的三个最大值,并将他们四个拼接起来
#                     my_sample = []
#                     for index in indices_per_row:
#                         tmp = new_sample[index]
#                         tmp = torch.sum(tmp,dim=0)
#                         my_sample.append(tmp)
#                     my_sample = torch.stack(my_sample)
#                     my_sample = my_sample+x_sample
#                 else:
#                     patch_id = torch.randperm(feat_reshape.shape[1], device=feats[0].device)
#                     patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))]  # .to(patch_ids.device)
#                 patch_id = torch.tensor(patch_id, dtype=torch.long, device=feat.device)
...
#             if self.use_mlp:
#                 mlp = getattr(self, 'mlp_%d' % feat_id)
#                 if patch_ids is None: #如果是空的，就证明是第一个netF
#                     x_sample = mlp(x_sample)
#                 else:
#                     x_sample= mlp(my_sample)
#             return_ids.append(patch_id)
#             x_sample = self.l2norm(x_sample)
#
#             if num_patches == 0:
...
#                     patch_id_tmp=set(patch_id.tolist())
#                     ramain_number = list(total_range-patch_id_tmp)
#                     random.shuffle(ramain_number)
#                     new_supplementary_id = torch.tensor(ramain_number)
#                     # 3.根据得到的不重复的剩余的数，获取新的id
#                     # new_supplementary_id = torch.randperm(len(ramain_number), device=feats[0].device)
#                     # new_supplementary_id = new_supplementary_id[:3*256]#
#                     new_supplementary_id = new_supplementary_id[:3*num_patches]#
